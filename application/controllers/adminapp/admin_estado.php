...
class Admin_Estado extends CI_Controller
{

  function __construct()
    {
		parent::__construct();
		$this->load->model('dai/estado_model');
 ...
 public function index()
    {
        ...
        $config['per_page'] = 7;
        $config['base_url'] = base_url().'index.php/adminapp/admin_estado/index/';
    ...
    //limit end
        ...
        if($manufacture_id !== false && $search_string !== false && $order !== false || $this->uri->segment(2) == true){ 
            ...
            */
            ...
            $data['count_products']= $this->estado_model->count($search_string, $order);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if($search_string){
                if($order){
                $data['products'] = $this->estado_model->get_all($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                 //$data['products'] = $this->proyecto_investigacion_model->get_all_proyectos($config['per_page'],$limit_end); 
                }else{
                $data['products'] = $this->estado_model->get_all($search_string, '', $order_type, $config['per_page'],$limit_end);           
                // $data['products'] = $this->proyecto_investigacion_model->get_all_proyectos($config['per_page'],$limit_end); 
                }
            }else{
                if($order){
                  $data['products'] = $this->estado_model->get_all('', $order, $order_type, $config['per_page'],$limit_end);        
                 //$data['products'] = $this->proyecto_investigacion_model->get_all_proyectos($config['per_page'],$limit_end); 
                }else{
                 $data['products'] = $this->estado_model->get_all('', '', $order_type, $config['per_page'],$limit_end);        
                //$data['products'] = $this->proyecto_investigacion_model->get_all_proyectos($config['per_page'],$limit_end); 
                }
            }
            ...
            //fetch sql data into arrays
            $data['count_products']= $this->estado_model->count_();
            $data['products'] = $this->estado_model->get_all('', '', $order_type, $config['per_page'],$limit_end);
            //$data['products'] = $this->proyecto_investigacion_model->get_all_proyectos($config['per_page'],$limit_end);        
        ...
        //load the view
        $data['main_content'] = 'admin/estado/list';
 ...
 public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if($this->estado_model->add($data_to_store)){
        ...
        //load the view
        $data['main_content'] = 'admin/estado/add';
        $this->load->view('includes/template', $data);  
    }
    ...
    public function update()
    {
        ...
        //product data 
        $data['product'] = $this->estado_model->get_by_id($id);

        //load the view
        $data['main_content'] = 'admin/estado/edit';
        $this->load->view('includes/template', $data);            
